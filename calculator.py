
# coding: utf-8

# # Calculator with the help of tkinter

# In[1]:


from tkinter import *
 
root = Tk()#this let us create a page
root.title("Calculator") #give the title 
Label1 = Label(root, text = "Calculator app")
Label1.grid(row=0, columnspan=8) #specifying the location
 
equa = ""
equation = StringVar() 
 
calculation = Label(root, textvariable = equation)
 
equation.set("Enter value : ")
 
calculation.grid(row=2, columnspan=8)
 
def btnPress(num):
    global equa
    equa = equa + str(num)
    equation.set(equa)  ##global helps in identifying the number of times the button is pressed. this will be added to th number or value assigned to the button.
    
 
def EqualPress():
    global equa
    total = str(eval(equa))
    equation.set(total)
    equa = ""

 
Button0 = Button(root, text="0", command =lambda:btnPress(0))
Button0.grid(row = 6, column = 2)
Button1 = Button(root, text="1", command = lambda:btnPress(1))
Button1.grid(row = 3, column = 1)
Button2 = Button(root, text="2", command = lambda:btnPress(2))
Button2.grid(row = 3, column = 2)
Button3 = Button(root, text="3", command = lambda:btnPress(3))
Button3.grid(row = 3, column = 3)
Button4 = Button(root, text="4", command = lambda:btnPress(4))
Button4.grid(row = 4, column = 1)
Button5 = Button(root, text="5", command = lambda:btnPress(5))
Button5.grid(row = 4, column = 2)
Button6 = Button(root, text="6", command = lambda:btnPress(6))
Button6.grid(row = 4, column = 3)
Button7 = Button(root, text="7", command = lambda:btnPress(7))
Button7.grid(row = 5, column = 1)
Button8 = Button(root, text="8", command = lambda:btnPress(8))
Button8.grid(row = 5, column = 2)
Button9 = Button(root, text="9", command = lambda:btnPress(9))
Button9.grid(row = 5, column = 3)
Plus = Button(root, text="+", command = lambda:btnPress("+"))
Plus.grid(row = 3, column = 4)
Minus = Button(root, text="-", command = lambda:btnPress("-"))
Minus.grid(row = 4, column = 4)
Multiply = Button(root, text="*", command = lambda:btnPress("*"))
Multiply.grid(row = 5, column = 4)
Divide = Button(root, text="/", command = lambda:btnPress("/"))
Divide.grid(row = 6, column = 4)
Equal = Button(root, text="=", command = EqualPress)
Equal.grid(row=6, column=3) ##create as many buttons as needed with mentioning the command.
 
root.mainloop() #finally we end the program. 

